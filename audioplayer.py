...
def create_round_button(parent, label, command=None):
    canvas = tk.Canvas(parent, width=64, height=64, bg="#122FAA", highlightthickness=0)
    canvas.create_oval(4, 4, 60, 60, fill="#32127A", outline="#444444")
...
def toggle_playlist():
    ...
    if playlist_panel and playlist_panel.winfo_exists():
        ...
        playlist_panel.configure(bg="#122FAA")
...
def toggle_transparency_slider():
    if getattr(window, "transparency_panel", None) and window.transparency_panel.winfo_exists():
        ...
        window.transparency_panel.configure(bg="#122FAA")
        window.transparency_panel.overrideredirect(True)
        window.transparency_panel.attributes("-topmost", True)
        label = tk.Label(window.transparency_panel, text="Прозрачность", font=("Segoe UI", 10), bg="#20B2AA", fg="white")
...
cover = tk.Label(window, text="Здесь могла быть ваша картинка", font=('Segoe UI', 14), bg="#20B2AA", fg="white")
cover.pack(pady=(20, 5))

progress = ttk.Scale(window, from_=0, to=100, orient="horizontal", length=200)
progress.pack(pady=5)

time_label = tk.Label(window, text="1:00", font=('Segoe UI', 10), bg="#20B2AA", fg="white")
time_label.pack(pady=(0, 10))

button_frame = tk.Frame(window, bg="#122FAA")
